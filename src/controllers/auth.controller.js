...
const register = async (req, res) => {
    ...
    try {
        ...
        if ( user) return res.status(400).send({message: "Email already exist try with a diff email id"});
        ...
        return res.status(500).send({message: "Sorry for register inconvenience please try again later"});
    }
}
